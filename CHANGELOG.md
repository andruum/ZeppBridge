...
- **应用起不来的时候，现在会说出原因。** 在此之前，Windows 上启动失败是完全无声的：没有窗口、没有提示、没有日志——只剩一个已经退出的进程留在通知区的图标，点它没任何反应。重装也没用，因为数据目录就在安装目录旁边，卸载不会动它。现在启动失败会弹一个对话框，把具体目录和系统错误写出来，并在数据旁边留下 `logs/startup-error.log`，可以直接附在反馈里。每次启动也会写 `logs/zeppbridge.log`（单份 2 MB，保留三份）——重放、压缩、托盘那些消息以前就写在那里，而打包后的 Windows 构建根本没地方显示它们。
- **Windows: a data folder the app cannot write to no longer kills the launch.** The `.msi` installs into `Program Files`, where a standard user has no write access; Controlled Folder Access and some antivirus products block it too. macOS and Linux had a fallback for exactly this and Windows did not. It now falls back to `%APPDATA%\zeppbridge\ZeppBridge\data`. The one case it refuses to do that quietly is when a database is already sitting in the blocked folder — starting somewhere else would look like every record had vanished, so it reports the two paths instead.
- **Windows：数据目录写不进去不会再直接让应用起不来。** `.msi` 装到 `Program Files`，普通用户对它没有写权限；受控文件夹访问和一些杀毒软件也会拦。macOS 和 Linux 本来就有这条退路，Windows 没有。现在会回退到 `%APPDATA%\zeppbridge\ZeppBridge\data`。唯一不悠悠回退的情况是那个写不进去的目录里已经有一个库——换个地方开会让人以为数据全没了，所以那时它会把两个路径都告诉你。
- **A firmware version is no longer displayed as if it were a device.** Some accounts' device lists carry no name field at all, and the app fell back to the only other text in the payload — the hardware version. The sidebar then showed data sources called `0.91.20.5` that did nothing when clicked, and a Bip 6 owner wrote in to say their watch "says 0.135.23.0". Those entries are real devices from your Zepp account; they now read "Not identified" and offer the model picker, and repeated bindings of the same watch collapse into one row.
- **固件号不会再被当成设备名显示。** 有些账号的设备列表里一个名字字段都没有，应用就取了报文里仅剩的那串文字——硬件版本号。于是侧栏里出现了叫 `0.91.20.5` 的数据来源，点进去没有任何东西；一位 Bip 6 用户写信说他的表「显示成 0.135.23.0」。那些条目是你 Zepp 账号里真实存在的设备，现在它们显示为「未识别」并给出型号指认入口；同一台表的多次绑定也不再占多行。
- **An error report can now carry the code the cloud refused you with.** This is the missing half of the refusal fix above: the app can already tell a refusal apart from unreadable data, but it deliberately does not decide on its own that a given code means "sign in again" — no failing code has ever been observed here. If you submit a report, it now includes that number, which data stream, and when. Never any text the cloud returned. That one integer is what will let the next release tell people with an empty-looking account to reconnect, instead of leaving them staring at nothing.
- **错误报告现在能带上云端拒绝你时的那个错误码。** 这是上面那条修复缺的另一半：应用已经能把「被拒绝」和「数据读不懂」分开，但它故意不自己断定某个码就是「要重新登录」——本地从来没观测到过任何一个失败码。你提交反馈时，现在会带上那个数字、哪条数据流、什么时候，不包括云端返回的文字。正是这一个整数，能让下一版对那些「账号看起来是空的」的人直接说「请重新连接」，而不是让他们对着一片空白干等。
...
- **FIT export and HAR import could not be opened at all.** Picking FIT on the export page, or importing a `.har` in settings, failed instantly with `Command plugin:dialog|open not allowed by ACL`. The permission list granted `dialog:allow-save` but not `dialog:allow-open`, and those are two separate grants — saving a JSON, CSV or GPX file worked, so nothing ever caught it, while the two paths that need to *pick* something (a folder for FIT, a file for HAR) could not run at all. FIT export was the headline feature of 2.1.0 and it shipped broken on every platform. Reported in [#28](https://github.com/lingcang728/ZeppBridge/issues/28). A test now scans the interface for every dialog method it calls and fails the build if the permission list does not match, in both directions.
- **FIT 导出和 HAR 导入根本点不开。** 在导出页选 FIT，或者在设置里导入 `.har`，会立刻失败并报 `Command plugin:dialog|open not allowed by ACL`。权限清单里写了 `dialog:allow-save`，没写 `dialog:allow-open` —— 这是两条独立的权限，而保存 JSON / CSV / GPX 用的是前者，所以一直是通的，没有任何东西发现问题；真正需要「挑一个东西」的那两条路（FIT 选文件夹、HAR 选文件）则完全跑不起来。FIT 导出正是 2.1.0 的主打功能，而它在所有平台上都是坏的。报告见 [#28](https://github.com/lingcang728/ZeppBridge/issues/28)。现在有一个测试会扫描界面里用到的每一个 dialog 方法，和权限清单双向对账，对不上就构建失败。
- **The all-day stress screen shows the whole day, not one number a day.** A user wrote in to say the stress meter "isn't 24/7", and they were right. Zepp's `all_day_stress` payload carries the entire day's curve — a reading every five minutes — in a field called `data`, and ZeppBridge was reading the daily average out of the same record and throwing the curve away. In this library that was 62,626 readings over 522 days, downloaded and then discarded. The curve is now stored, exported, and drawn on the body-status screen as a 24-hour chart. Nothing needs re-syncing: the raw payloads were kept all along, so the first launch after updating re-reads them and fills in the history. The four band shares — how much of the day was relaxed, normal, medium or high — were being dropped too, for a duller reason: the code looked for `relaxPct` while every real payload says `relaxProportion`, so those four values had never once been written to the database.
- **全天压力界面现在显示一整天，而不是一天一个数。** 有用户来信说压力「不是 24/7」——他是对的。Zepp 的 `all_day_stress` 报文里有个 `data` 字段，装的是当天整条曲线，五分钟一个点；而 ZeppBridge 从同一条记录里取走了日平均值，把曲线扔了。在本机这份库里，那是 522 天、62 626 条读数，下载下来又丢掉。现在这条曲线会入库、会进导出，也会在身体状态页画成 24 小时曲线。不需要重新同步：原始报文一直都留着，更新后第一次启动会重新解析它们，历史一并补上。四项区间占比（一天里放松 / 正常 / 中等 / 高各占多久）也一直在被丢，原因更平淡：代码找的是 `relaxPct`，而真实报文里写的是 `relaxProportion`，于是这四个值从来没有写进过数据库。
- **The stress curve now really breaks where nothing was measured.** The card's own footnote said gaps are left empty; the chart did not do that. Hours with no samples simply had no points, and the chart joined the two ends of a gap with a straight line — a line nobody ever measured. The watch samples every five minutes, so a gap longer than three sampling intervals now inserts a real break. Three rather than one because five minutes is only nominal: neighbouring readings 5–8 minutes apart are common, and breaking at one interval would shred a normal curve into dashes. The tooltip no longer reports a number on a break, and the axis range is taken from real readings instead of possibly landing on an inserted gap.
- **压力曲线现在在没采样的时段真的断开。** 卡片脚注写的是「没采样的时段留空」，画出来不是：缺的那几个小时没有点，图表就把缺口两端连成一条直线——一条从来没测过的直线。手表每五分钟测一次，所以超过三个采样间隔的缺口现在会真的断开。三个而不是一个：五分钟只是标称值，实测相邻两点差 5–8 分钟很常见，卡到一个间隔会把正常曲线打成虚线。断点上 tooltip 不再报一个数出来，x 轴范围也改从真实读数取。
...
- **A `.fit` now carries the watch's own step count.** The session had no total step field, so importers estimated one — OPPO Health turned a 0.83 km walk into 1,274 steps while the cloud summary had the real number all along. It is written for running, walking and hiking only, halved to match FIT's definition of a cycle as one full stride; cycling is left alone, where a cycle is a crank turn and a step count would be meaningless.
- **`.fit` 现在带上了手表自己数的步数。** session 里没有总步数字段，导入方只能自己估——OPPO 健康把一次 0.83 公里的健走估成 1274 步，而云端汇总里一直有真实步数。只在跑步、健走、徒步上写，并且除以二以对上 FIT「一个 cycle 是一整步」的定义；骑行不写，那里一个 cycle 是曲柄转一圈，塞进步数只会得到一个假的踏频总数。
- **Linux: the Flatpak has a tray icon again.** Tauri loads `libayatana-appindicator3` at runtime, and the GNOME Flatpak runtime does not ship it. A native install can be fixed with one `apt install`; a Flatpak user has no such option, so the manifest now builds the three libraries into the sandbox itself.
- **Linux：Flatpak 又有托盘图标了。** Tauri 在运行时才去 dlopen `libayatana-appindicator3`，而 GNOME 的 Flatpak runtime 里没有这个库。原生安装的用户 `apt install` 一下就能补上，Flatpak 用户没有这条路，所以现在由 manifest 把那三个库直接构建进沙箱。
- **Linux: the AppImage no longer carries its own Wayland libraries.** [#32](https://github.com/lingcang728/ZeppBridge/issues/32) stayed broken for the AppImage after the Flatpak was fixed, and the reporter's own guess turned out to be exactly right: it was the Wayland display libraries. Dumping the bundle showed it shipping `libwayland-client`, `libwayland-cursor`, `libwayland-egl` and `libwayland-server`, and `libwayland-client` has to agree with the compositor actually running on your machine — which is why an AppImage built on the reporter's own machine ran while the one built in CI did not. Those four are now dropped from the bundle so the host's copies are used, and the app additionally turns off accelerated compositing when it detects it is running from an AppImage, leaving the deb, rpm and Flatpak builds untouched. **This one is unverified**: it builds and repacks in CI, but nobody has yet confirmed it opens on a machine that was failing.
- **Linux：AppImage 不再自带 Wayland 库。** Flatpak 修好之后 [#32](https://github.com/lingcang728/ZeppBridge/issues/32) 里的 AppImage 仍然打不开，而报告者自己的猜测正好是对的：问题就出在 Wayland 显示库上。把包里的库整份打印出来之后可以看到，它带了自己的 `libwayland-client`、`libwayland-cursor`、`libwayland-egl` 和 `libwayland-server`，而 `libwayland-client` 必须和你机器上真正在跑的合成器对得上——这也正好解释了为什么报告者自己编的能跑、CI 编的不能。现在这四个不再打进包里，改用宿主机自己的；应用另外会在检测到自己从 AppImage 启动时关掉加速合成，deb / rpm / Flatpak 三条渠道不受影响。**这一条没有被验证过**：CI 能构建、能重新打包，但还没有人在原本打不开的机器上确认它现在能开。

### Added / 新增

- **The workout page shows how long you spent in each heart-rate zone.** The watch computes this against the zone boundaries you set on it and sends it with every workout; it has been decoded and stored since 2.1.0 but never drawn. It now appears under the per-second chart as a stacked bar with the duration and share of each of the six zones. The page says plainly where the boundaries come from — your watch, not us — because the training-status page uses a different zone model you pick yourself, and the two will not agree.
- **运动详情页显示你在各心率区间待了多久。** 手表按你在表上设定的区间边界算好这件事，并随每条运动一起下发；从 2.1.0 起它就已经被解析入库，只是界面从来没画过。现在它出现在逐点曲线下面，一条堆叠条加六段的时长和占比。页面上写明了边界的来处——是你的手表，不是我们——因为训练状态页用的是另一套你自己选的区间模型，两边数字对不上是正常的。
- **Fifteen more numbers Zepp was already sending are now kept.** From the daily summary: sleep-period HRV and resting heart rate, your personal HRV and resting-heart-rate baselines, the AHI baseline, and the step, calorie and active-minute goals you set yourself. From the wellness stream: the seven-day rolling PAI total — the big number on Zepp's own screen, which had never been read — plus how long you spent in each of the three PAI zones and the heart-rate floor of each. Every field got its own sentinel rule rather than sharing one: `255` means "not measured" for scores, `-1` for the AHI baseline, `0` for a goal you never set — but zero minutes in a PAI zone is a real reading and is kept.
- **云端一直在给、而我们一直没取的十五个数字，现在存下来了。** 来自每日汇总的：睡眠期 HRV 与静息心率、你个人的 HRV 与静息心率基线、AHI 基线，以及你自己设定的步数、热量和活动分钟目标。来自 wellness 的：七天滚动的 PAI 总分——Zepp 自己界面上那个大数字，此前从没被读过——加上三档 PAI 区间各待了多久、以及每一档的心率下限。每个字段各有一条哨兵规则，绝不共用：`255` 是分数类的「没测到」、`-1` 是 AHI 基线的、`0` 是目标没设——但 PAI 区间里的 0 分钟是**真读数**，会被存下来。
- **A run's first half and second half are compared by pace per heartbeat.** It measures how many metres you covered per heartbeat, and whether that fell in the second half. The number is easy to poison — traffic lights, hills, intervals and GPS drift all make the two halves incomparable — so when a run does not qualify you get the reason instead of a number computed anyway: shorter than 20 minutes, speed too variable, too few usable samples, or a workout type this has not been validated against. It splits at the halfway point in *time*, not by sample count, so a gap in the recording cannot hand one half twice the duration of the other.
- **一次跑步的前后半程按「每一拍心跳跑出多少米」对比。** 后半程比前半程低，说明维持同样的速度要花更多心跳。这个指标特别容易被路况污染——红绿灯、爬坡、间歇、GPS 漂移都会让两半程根本不可比——所以条件不满足时给的是原因，不是一个硬算出来的数字：短于 20 分钟、速度变异太大、可用采样不足、或者这个运动类型还没有用真实数据验过阈值。它按**时间**的中点切，不按样本个数切，这样中间掉了一段采样也不会把两边的时长切得完全不一样。
...
- **Elliptical is a sport now, and four more watches are recognised by number.** A user wrote in to say Zepp's workout code 12 is the elliptical trainer, and that is the whole reason it was added — the catalogue takes written evidence, not vote counts, and other unknown codes with far more reports behind them were left alone. Elliptical did not exist in the catalogue at all before this, so until now you could not even correct a workout to it by hand. Upgrading replays your stored workouts, so records already filed as `unknown:12` become elliptical without re-syncing. Four device numbers reached a second independent report and are now identified automatically: Active MAX, Bip MAX, Active 3 Premium and T-Rex 3 Pro. Two numbers where reporters genuinely disagree were deliberately left out — a majority is not evidence when getting it wrong renames the watch for everyone who owns that model.
- **椭圆机进目录了，另有四款表可以靠编号自动认出。** 有用户来信说 Zepp 的运动编号 12 就是椭圆机，收录它的理由就是这一句——目录要的是文字证据，不是票数，其它报告数多得多的未知编号一个都没动。椭圆机此前在目录里根本不存在，所以在此之前你连手动把一条记录改成椭圆机都做不到。升级会重放你已有的运动记录，所以已经存成 `unknown:12` 的那些不用重新同步就会变成椭圆机。四个设备编号凑够了第二份互相独立的报告，现在能自动识别：Active MAX、Bip MAX、Active 3 Premium、T-Rex 3 Pro。两个报告者之间存在真实分歧的编号刻意没收——收错一个编号会让所有同款用户的设备名一起错掉，这种时候多数票不算证据。
- **Amazfit Balance 2 XT is in the model list.** [#42](https://github.com/lingcang728/ZeppBridge/issues/42) reported it missing, which meant its owners could not even correct their device by hand. It ships without a product photo and falls back to a placeholder outline: the watch is currently sold only through retailers, with no page on Amazfit's own site to take an image from, and showing the Balance 2's photo instead would just be a picture of a different watch.
- **型号列表里有 Amazfit Balance 2 XT 了。** [#42](https://github.com/lingcang728/ZeppBridge/issues/42) 报告它不在列表里，而这意味着用这款表的人连手动指认都做不了。它暂时没有产品图，会显示一个占位轮廓：这款表目前只在零售渠道上架，Amazfit 官方站上没有可以取图的产品页，而拿 Balance 2 的图顶上去只是给你看一张别的表的照片。

### Docs / 文档

- **Moving a library from Windows or macOS to Linux now has instructions.** [#40](https://github.com/lingcang728/ZeppBridge/issues/40) copied a data folder across and found the database read fine but the command line would not sync. The database travels; the token does not, and never could — it lives in Credential Manager, the Keychain or the Secret Service, bound to that machine, never in a file. `auth.json` holds only the user ID and region host, so a copied folder always arrives with the metadata and no secret. The Linux guide now walks through the three ways to put a token on the new machine, and the error message names them instead of only saying "please pair again".
- **把库从 Windows 或 macOS 搬到 Linux，现在有说明了。** [#40](https://github.com/lingcang728/ZeppBridge/issues/40) 把数据文件夹拷了过去，数据库读出来了，命令行却同步不了。数据库能搬，令牌搬不过来、也从来不可能——它在凭据管理器 / 钥匙串 / Secret Service 里，和那台机器绑定，从来不在文件里。`auth.json` 只有用户 ID 和区域地址，所以拷过来的文件夹一定是「元数据齐了、密钥没有」。Linux 指南现在把三条配置凭据的路都写清楚了，错误文案也直接点名它们，不再只说一句「请重新配对」。
