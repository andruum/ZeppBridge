import { defineMessages } from '../i18n';

/*
 * 设置页的文案。
 *
 * 单独放一个文件的理由和 `Explore.i18n.ts` 一样：这一页本来就有两千行，
 * 再往 `<script setup>` 里塞四百行文案，逻辑会被推到看不见的地方。
 * 只被 Settings.vue import，所以跟着它的 chunk 走，不进首屏。
 */
export const settingsMessages = defineMessages(
  {
    // ── 页头 ──
    title: '设置',
    intro: '管理认证方式、同步行为、隐私与默认导出偏好，确保本地数据安全。',
    retry: '重试',
    distanceUnitLabel: '距离单位',
    displayPrefsTitle: '语言与格式',

    // ── 1. 认证方式 ──
    authTitle: '1. 认证方式',
    authWebTitle: '官方网页登录',
    authWebSub: '通过官方页面登录，自动抓取 appToken',
    authCancelLogin: '取消登录',
    authInUse: '当前使用',
    authOpening: '正在打开…',
    authRetry: '重试连接',
    authUse: '使用',
    authHarTitle: 'HAR 导入',
    authHarSub: '适合高级用户与调试，快速导入 HAR 文件',
    authManualTitle: '手动填写',
    authManualSub: '手动填写 appToken、user_id 等信息',
    authCollapse: '收起',
    manualFormTitle: '手动输入认证信息',
    manualFormHint: '从 mitmproxy/Charles 抓包或浏览器开发者工具获取。需要三个字段：',
    manualTokenPlaceholder: '从 HTTP 请求头 apptoken 字段复制',
    manualUserIdPlaceholder: '从 URL 路径 /users/{user_id}/ 提取',
    manualSaving: '保存中...',
    manualSave: '保存认证',
    cancel: '取消',

    // ── 2. 账户与区域 ──
    accountTitle: '2. 账户与区域',
    accountLine: (region: string, lastSync: string) => `区域 ${region} · 上次同步 ${lastSync}`,
    verifyAndSync: '验证并同步',
    reauthenticate: '重新认证',

    // ── 3. 设备 ──
    devicesTitle: '3. 连接设备 / 数据来源',
    identifying: '正在识别…',
    identifyDevices: '重新识别设备',
    deviceErrorPrefix: '设备识别：',
    noDevices: '尚未识别实体设备；Zepp Cloud 仍可作为云服务同步。',
    deviceFirmware: (firmware: string) => `固件 ${firmware}`,
    deviceLatestData: '最近数据',
    deviceIdLine: (masked: string) => `设备 ID ${masked}`,
    viewOrChange: '查看 / 换型号',
    unknownDeviceTitle: '检测到未识别设备',
    unknownDeviceBodyA: '有些 Zepp 账号的设备响应里',
    unknownDeviceNoName: '没有任何产品名字段',
    unknownDeviceBodyB: '，只有内部编号，本机无法推断型号——这种情况下「重新识别设备」再点多少次也不会变。你可以在上面直接指认型号：那会被如实标注成「你指认的型号」，不会伪装成自动识别结果。',
    unknownDeviceReport: '提交错误报告可以帮我把这台设备的编号补进内置目录，之后所有人都不用手动指认。报告只含固定白名单字段，无需 GitHub 账号。',

    // ── 错误报告表单 ──
    reportWhat: '要反馈什么',
    reportWhatHint: '（本机没自动检测到问题时，选一个就能提交）',
    reportCategoryPlaceholder: '不指定（只发送自动检测到的问题）',
    reportCategoryAria: '要反馈的问题类型',
    reportNote: '补充说明',
    reportNoteHint: '（选填，但很有用）',
    reportNotePlaceholder: '例如：我的表是 Amazfit Balance 2，但这里显示未识别；或者：户外骑行被识别成了未知运动。',
    reportNoteCounter: (used: number, max: number) =>
      `${used} / ${max} · 发送前会自动去掉本机路径、邮箱和长串标识`,
    reportSubmitting: '正在安全提交…',
    reportSubmit: '提交错误报告',
    reportDoneTitle: '已收到，谢谢',
    reportDoneLine: (id: string, at: string) => `报告编号 ${id}，提交时间 ${at}。`,
    reportDoneNote: '发出去的就是上面列出的那几类字段和你写的那段说明，没有别的。',
    reportConfirm: '只会发送应用版本、系统类型、解析器版本、未识别设备的产品级提示与字段结构、固件版本、型号类编号（deviceSource / deviceType，只有整数，描述的是「哪一款表」而不是「哪一台表」）、未知运动编号和数量、云端最近一次拒绝请求的数字错误码（只有编号、哪条数据流和时间，不包括云端返回的文字），以及你在上面写的那段说明（会自动去掉本机路径、邮箱和长串标识）。不会发送 Zepp 账号、Token、序列号、设备 ID、MAC 地址、GPS、健康数值或原始响应。确认提交吗？',
    reportFailed: '错误报告提交失败',

    // ── 能力清单 ──
    capabilityTitle: '你的设备能提供什么',
    capabilityIntro: '以下是 ZeppBridge 目前能从你的账号读到的数据。这份清单在同步时自动更新，无需手动操作。',
    lampOn: (count: number) => `已获取 ${count}`,
    lampPending: (count: number) => `云端有、本机未收录 ${count}`,
    lampOff: (count: number) => `暂未获取 ${count}`,
    capabilityEmptyTitle: '尚未同步',
    capabilityEmptyBody: '完成一次同步后这里会亮起来。',
    probeSummary: '接口诊断详情',
    probeNote: '「暂未获取到」不等于设备不支持：Zepp 的接口对不存在的数据流也返回空响应，只有接口明确拒绝时才会写成「你的设备不提供」。',
    probing: '正在检测…',
    probeRun: '立即重新检测',
    probedToday: '今天检测',
    probedDaysAgo: (days: number) => `${days} 天前检测`,
    probeRecords: (records: number, latest: string) => `${records} 条${latest ? `，最新 ${latest}` : ''}`,
    probeEmpty: '无数据',
    probeRefused: '接口拒绝',
    probeFailed: '请求失败',

    // ── 未识别运动编号 ──
    codesTitle: '未识别的运动编号',
    codesUnnamed: (count: number) => `${count} 个还没有名字`,
    codesIntro: 'Zepp 的自定义训练模板只给编号、不给名字，内置目录里也查不到它们。与其猜一个运动名塞给你，不如你给这个编号起一次名字——之后所有同编号的记录都会用它，并且在运动详情里如实标注成「你起的名字」。',
    codeNumber: (code: number) => `Zepp 编号 ${code}`,
    codeRecords: (count: number) => `本机 ${count} 条记录会一起改名`,
    codeShownAs: (label: string) => `现在显示为「${label}」`,
    codeShownAsUnknown: (code: number) => `现在显示为「未识别运动（编号 ${code}）」`,
    codeInputAria: (code: number) => `编号 ${code} 的自定义名称`,
    codeInputPlaceholder: '给它起个名字，例如：我的核心训练',
    codeSaving: '保存中…',
    codeSave: '保存',
    codeFootnote: '名字只保存在本机，不会回传 Zepp，也不会被重新解析覆盖。留空并保存即可清除。',
    codeSaved: (code: number, label: string) => `编号 ${code} 以后都显示为「${label}」。`,
    codeCleared: (code: number) => `已清除编号 ${code} 的自定义名称。`,
    codeSaveFailed: '无法保存自定义运动名称',
    codeSuggestions: ['力量训练', '核心训练', 'HIIT', '拉伸放松', '康复训练', '自定义训练'],

    // ── 4. 隐私与安全 ──
    privacyTitle: '4. 隐私与安全',
    privacyDbTitle: '本地数据库未加密',
    privacyDbBody: '健康数据以明文 SQLite 保存在程序目录的 data 文件夹，依赖 Windows / macOS 的账户与磁盘加密保护。ZeppBridge 不提供整库加密，也不会假装提供。',
    privacyTokenTitle: 'Zepp 令牌默认保存在系统凭据存储',
    privacyTokenBody: '默认使用 Windows 凭据管理器 / macOS 钥匙串 / Linux 密钥环。macOS 和 Linux 可显式选用仅当前用户可读写的明文凭据文件；Linux 也支持环境变量。auth.json 只含账号与区域等元数据，令牌不会写进日志、数据导出或错误报告。',
    privacyTelemetryTitle: '没有埋点，没有使用统计',
    privacyTelemetryBody: '应用不会自动上报任何使用行为。只有你亲手点击「提交错误报告」时，才会发送下面列出的那几类脱敏字段。',
    privacyModalLink: '查看本地隐私与脱敏原则',
    privacyReportTitle: '设备或运动没有识别？',
    privacyReportBody: '无需注册 GitHub 或复制数据。确认后只把产品级字段结构、固件版本、型号类编号（整数，只说明是哪一款表）、未知运动编号和数量发送到 ZeppBridge 的私有错误报告库；绝不发送账号、Token、序列号、设备 ID、MAC 地址、GPS、健康数值、原始响应或本机路径。',

    // ── 5. MCP ──
    mcpTitle: '5. MCP（让 AI 工具直接问本机数据）',
    mcpBadge: '只读 · 不监听端口',
    mcpSkip: '不知道 MCP 是什么可以跳过，它不影响 ZeppBridge 的任何功能。',
    mcpCompareA: '一句话说：「交给 AI」是你导出数据粘给 AI；MCP 是',
    mcpCompareStrong: '让 AI 自己来问',
    mcpCompareB: '——配好之后直接对它说「看看我最近一个月的睡眠」，它自己去你本机的库里查。只对装在你电脑上的 AI 编程工具有用（Claude Code、Codex、Grok 这类）。',
    mcpAskA: '配置步骤因工具而异，与其在这里写一大篇，不如',
    mcpAskStrong: '把下面这段复制给你正在用的 AI',
    mcpAskB: '，让它照着你的机器给你指引。',
    mcpCopyPrompt: '复制这段去问 AI',
    mcpCopyConfig: '只复制配置片段',
    mcpToolsLead: '配好之后，AI 能问到这五件事：',
    mcpFootA: ' 随 Release 的工具压缩包一起分发，和桌面应用同一个版本。它读的是同一个本机数据库，所以看到的数据和你在这个界面里看到的完全一致。',
    mcpPromptCopied: '已复制。粘给你正在用的 AI，它会照着你的机器给出配置步骤。',
    mcpPromptCopyFailed: '复制失败，请手动选中上面那段文字。',
    mcpConfigCopied: '配置已复制。把 command 换成你本机 zeppbridge-mcp 的实际路径。',
    mcpConfigCopyFailed: '复制失败，请手动选中上面的配置。',
    mcpToolListWorkouts: '运动记录列表，最新在前',
    mcpToolWorkoutInsight: '一次运动和你自己基线的比较',
    mcpToolMetricSeries: '按天的指标序列，每条带单位',
    mcpToolSleepDetail: '一晚睡眠的分期明细',
    mcpToolDataHealth: '每条流的抓取/解析/写入状态',
    mcpSetupPrompt: `我在用一个叫 ZeppBridge 的 Windows 桌面应用，它把我的 Amazfit / Zepp 手表数据同步到本机的一个 SQLite 数据库里。
它附带一个 MCP 程序（zeppbridge-mcp），我想把它配置到你这里，这样你就能直接查我的运动和健康数据，不用我每次导出再粘贴。

关于它的已知信息：
- MCP 程序要从 ZeppBridge 的 GitHub Release 页下载 zeppbridge-tools 压缩包，解压后里面有 zeppbridge-mcp 可执行文件。我可能还没下载。
- 它是 stdio 类型的 MCP server，只读本机数据库，不联网、不监听端口、不需要任何 token 或 API key。
- 典型配置形状是：{"mcpServers": {"zeppbridge": {"command": "<zeppbridge-mcp 的完整路径>", "args": []}}}
- 它提供六个只读工具：list_workouts（运动列表）、get_workout_insight（单次运动与个人基线的比较）、get_metric_series（按天的指标序列）、get_sleep_detail（一晚睡眠明细）、get_sleep_for_date（按本地日期和 IANA 时区查询睡眠）和 get_data_health（每条数据流的抓取/解析/写入状态）。

请告诉我：
1. 针对你（我现在正在用的这个工具）具体应该把配置写到哪个文件、用什么命令添加；
2. Windows 上路径要怎么写（反斜杠要不要转义）；
3. 配完怎么验证生效。

如果你需要我提供什么信息（比如我用的是哪个客户端、文件放在哪），直接问我。`,
    mcpConfigPathPlaceholder: '<zeppbridge-mcp 的路径>',

    // ── 6. 本地数据保留 ──
    retentionTitle: '6. 本地数据保留',
    retentionLabel: '保留时长',
    retentionAria: '本地数据保留天数',
    retentionNote: (days: number) => `保留最近 ${days} 天的本地数据；清理在每次`,
    retentionNoteStrong: '成功同步之后',
    retentionNoteTail: '执行，不会在后台自行发生。',
    retentionCutoff: (date: string) => `下次成功同步后，${date} 以前的数据会被清理`,
    cleaningUp: '正在清理…',
    cleanupNow: '立即清理',
    reprocessing: '正在解析…',
    reprocessNow: '重新解析',
    days: (days: number) => `${days} 天`,
    lastDays: (days: number) => `最近 ${days} 天`,

    // ── 7. 导出与补拉偏好 ──
    exportTitle: '7. 导出与补拉偏好',
    defaultFormatLabel: '默认导出格式',
    defaultFormatAria: '默认导出格式',
    historyRangeLabel: '历史补拉范围',
    historyRangeAria: '历史补拉天数',
    exportNote: '设置「交给 AI」页面的默认格式与云端补拉窗口。',
    startBackfill: '开始历史补拉',
    formatJsonHint: '结构化数据',
    formatCsvHint: '表格数据',
    formatGpxHint: '运动轨迹',

    // ── 8. 软件更新 ──
    updateTitle: '8. 软件更新',
    updateSub: '每天最多静默检查一次，也可随时手动检查。',
    updateChecking: '检查中…',
    updateCheck: '检查更新',
    updateCurrent: (version: string) => `当前 ${version}`,
    updateVersion: (version: string) => `版本 ${version}`,
    buildStamp: (stamp: string) => `构建 ${stamp}`,
    updateVersionLoading: '读取中',
    updateSeeNotes: '看看更新了什么',
    updateStatusIdle: '尚未检查',
    updateStatusChecking: '正在检查 GitHub Release',
    updateStatusAvailable: (version: string) => `发现新版本 ${version}`,
    updateStatusDownloading: '正在下载更新',
    updateStatusDownloadingPercent: (percent: number) => `正在下载 ${percent}%`,
    updateStatusInstalling: '正在安装，完成后会自动重启',
    updateStatusFailed: '更新失败',
    updateStatusUpToDate: '当前已是最新版本',
    updateStatusUnmanaged: '由包管理器管理更新',
    updateUnmanagedHint: (version: string) => `当前 ${version}。这个安装包由 Flatpak 或系统包管理器更新：`
      + `flatpak update com.zeppbridge.app，或用发行版的包管理器升级。`,
    releaseNotesEmpty: '本次 Release 未填写更新说明。',
    updateModalTitle: (version: string) => `ZeppBridge ${version} 更新了什么`,
    updateModalCurrent: (version: string) => `你现在是 ${version}`,
    updateModalUnknownVersion: '未知版本',
    updateModalReleased: (date: string) => ` · 发布于 ${date}`,
    updateInstalling: '正在安装…',
    updateDownloading: '正在下载更新',
    updateInstallNote: '安装完应用会自己重启，本地健康数据不会被删除。',
    updateDownloadNoteTail: ' · 下载完会自动安装，这期间可以继续读上面的更新说明',
    updateDownloadNote: '下载完会自动安装，这期间可以继续读上面的更新说明。',
    updateFailedPrefix: (reason: string) => `更新失败：${reason}`,
    updateRestartNote: '安装时应用会自动重启，本地健康数据不会被删除。',
    updateBackground: '在后台继续',
    updateLater: '稍后再说',
    updateRetry: '重试',
    updateInstall: '下载并安装',

    // ── 9. 自动同步 ──
    syncTitle: '9. 自动同步',
    syncDescA: (minutes: number) => `应用打开期间每 ${minutes} 分钟自动同步云端记录`,
    syncDescB: '保持开启可获得连续的时序数据。',
    syncIntervalAria: '自动同步间隔',
    minutes: (minutes: number) => `${minutes} 分钟`,
    syncOn: '同步已开启',
    syncOff: '同步已关闭',
    syncing: '正在同步…',
    syncNow: '立即同步',

    // ── 高级与维护 ──
    advancedTitle: '高级与维护',
    advancedSub: '缩放、数据文件夹与认证清除，仅在需要时使用。',
    scaleLabel: '界面缩放',
    scaleNote: '100% 为设计基准，也可使用 Ctrl + / Ctrl -。',
    dataAuthLabel: '数据与认证',
    dataAuthNote: (days: number) => `数据保存在程序目录的 data 文件夹，当前保留 ${days} 天。`,
    openDataFolder: '打开数据文件夹',
    clearAuth: '清除认证',
    logout: '退出账号',
    logoutHint: '只退出账号，本机已同步的历史数据全部保留。重新登录后可以接着同步。',
    logoutNoMultiAccount: '暂不支持多账号切换：退出后再用另一个账号登录，两个账号的记录会存进同一个本机库。',
    healthCheckLabel: '数据健康检查',
    healthCheckNote: '每条数据流从云端取回、被解析、写进本机这三步分别走到哪一步，覆盖了哪些日期，来自哪个来源。平时不需要看；同步结果和你预期对不上时来这里找原因。',
    healthCheckOpen: '打开数据健康检查',
    compactLabel: '压缩历史报文',
    compactNoteA: '云端原始报文是这个库里最占地方的东西，它们是 JSON 文本，压缩后通常只剩五分之一。',
    compactNoteStrong: '这件事默认自动做',
    compactNoteB: '：装完新版本第一次启动时，后台会把存量报文压掉，顶部会显示「正在压缩」，压完自动消失。这个按钮只是让你手动再跑一次（比如上次被中断了）。压之前会先解压回来逐字比对，对不上的那条就跳过不动——原始报文是重放的唯一依据，宁可不压。压完会执行一次 VACUUM，磁盘上的文件才会真正变小。',
    compacting: '正在压缩…（大库需要几分钟）',
    compactRun: '压缩历史报文',
    backupLabel: '数据库快照与恢复',
    backupNote: '灾难恢复用的整库副本，只能由 ZeppBridge 自己读回来。数据库升级前会自动生成一份，平时不需要手动做。',
    localApiLabel: '本机 REST API',
    localApiNote: '给本机上的其他程序（脚本、看板、自建工具）读取已标准化的运动序列 JSON 用。如果你没有这类需求，保持关闭即可。',
    syncDiagnostics: '同步诊断',
    noSyncDiagnostics: '尚无同步诊断。',

    // ── 本机 REST API ──
    apiTitle: '本机 REST API',
    apiSub: '让本机上的其他程序读取已标准化的运动序列 JSON。默认关闭，需要你显式启用。',
    apiListening: '正在监听',
    apiEnabledNotListening: '已启用但未监听',
    apiOff: '已关闭',
    apiToggleTitle: '启用本机 API',
    apiToggleSub: (address: string) => `开关立即生效，不需要重启应用；关闭后 ${address} 会立刻释放。`,
    apiToggleAria: '启用本机 REST API',
    apiCopyExample: '复制带鉴权示例',
    apiTokenLabel: '访问令牌',
    apiHide: '隐藏',
    apiShow: '显示',
    apiCopy: '复制',
    apiRegenerate: '重新生成',
    apiAuthNoteA: '每个请求都必须带 ',
    apiAuthNoteB: '，否则返回 401。重新生成后旧令牌立即失效。',
    apiBindNote: '仅绑定 127.0.0.1，只读、不开放浏览器跨域、不返回任何凭据；退出 ZeppBridge 后停止。',
    apiTokenReadFailed: '无法读取本机 API 访问令牌',
    apiEnabled: '本机 API 已启用，无需重启应用。',
    apiDisabled: '本机 API 已关闭，端口已释放。',
    apiToggleFailed: '无法切换本机 API',
    apiTokenCopied: '访问令牌已复制到剪贴板。',
    apiTokenCopyFailed: '无法写入剪贴板，请点击「显示」后手动复制。',
    apiRegenerateConfirm: '重新生成后，旧令牌会立即失效，所有已经配置过它的本机程序都需要更新。确定吗？',
    apiTokenRegenerated: '已生成新的访问令牌，旧令牌已失效。',
    apiRegenerateFailed: '无法重新生成访问令牌',
    apiExampleCopied: '带鉴权的调用示例已复制（其中包含你的访问令牌）。',
    apiExampleCopyFailed: '无法复制调用示例，请手动拼接接口地址与 Authorization 头。',

    // ── 隐私弹窗 ──
    privacyModalTitle: 'ZeppBridge 本地隐私保障原则',
    privacyPoint1Title: '1. 本地处理优先：',
    privacyPoint1: '所有健康与运动时序数据仅存储在本地 SQLite 数据库中，解析与脱敏完全在本地完成。',
    privacyPoint2Title: '2. 认证凭据严格隔离：',
    privacyPoint2: 'App Token 与 User ID 等凭据不与任何第三方分享，AI 导出时自动执行不可逆脱敏。',
    privacyPoint3Title: '3. 敏感定位控制：',
    privacyPoint3: 'GPS 经纬度数据默认不注入 AI 剪贴板，严格保障家庭与常用运动路线隐私。',
    privacyPoint4Title: '4. 错误报告由你决定：',
    privacyPoint4: '只有点击并确认「提交错误报告」后，才发送固定白名单的产品级诊断；不会发送账号、设备 ID、运动详情或健康数据，也不会自动创建 GitHub Issue。',
    privacyPoint5Title: '5. 透明开源：',
    privacyPoint5: '端到端代码开源，无暗中网络回传逻辑。',
    privacyModalOk: '我知道了',
    closeDialog: '关闭对话框',

    // ── 状态与提示 ──
    connExtracting: '正在提取登录信息',
    connVerifying: '正在验证',
    connWaiting: '等待登录',
    connFailed: '登录失败',
    unidentified: '未识别',
    unidentifiedInitial: '未',
    notProvided: '未提供',
    noRecords: '尚无记录',
    timeUnknown: '时间未知',
    cloudService: '云服务',
    refreshFailed: (reason: string) => `重新识别失败，已回退到本机缓存${reason}`,
    refreshFailedReason: (reason: string) => `：${reason}`,
    refreshFailedPeriod: '。',
    refreshDone: (count: number) => `设备识别完成，共发现 ${count} 个实体设备。`,
    refreshNoNewList: '未获得新的设备列表，当前显示本机缓存。',
    loginIncomplete: '登录未完成',
    loginWindowFailed: '无法打开登录窗口',
    loginCancelFailed: '无法取消登录',
    harFilter: 'HAR 文件',
    harImported: 'HAR 文件导入成功，认证信息已保存。',
    harImportFailed: 'HAR 导入失败',
    filePickerFailed: '无法打开文件选择器',
    fillAllFields: '请填写所有必填字段',
    manualAuthDone: '手动认证成功，认证信息已保存。',
    manualAuthFailed: '手动认证失败',
    verifyFailed: '验证未完成',
    clearAuthConfirm: '确定退出账号吗？\n\n只清除登录凭据，本机已同步的历史数据全部保留。\n\n注意：暂不支持多账号切换——换一个账号登录会把两个账号的记录写进同一个本机库。',
    authCleared: '已退出账号。本机已同步的历史数据仍然保留。',
    clearAuthFailed: '无法清除认证信息',
    reprocessed: (count: number) => `本地数据已重新解析，共 ${count} 条标准化记录；云端同步时间未改变。`,
    reprocessFailed: '重新解析本地数据失败',
    cleanupConfirm: (days: number) => `确定清理 ${days} 天以前的本地数据吗？此操作无法撤销。`,
    cleanupDone: (days: number) => `已清理 ${days} 天以前的数据。`,
    cleanupFailed: '清理旧数据失败',
    openFolderFailed: '无法打开数据文件夹',
    nothingToCompact: '没有需要压缩的历史报文——它们已经是压缩形态了。',
    compactSkipped: (count: number) => `，跳过 ${count} 条（压完没变小或校验没通过）`,
    compactDone: (count: number, before: string, after: string, saved: string, skipped: string) =>
      `已压缩 ${count} 条报文，${before} → ${after}，省下 ${saved}${skipped}。`,
    compactFailed: '压缩历史报文失败',
    retentionConfirm: (days: number) => `下次成功同步将删除 ${days} 天以前的本地数据，不可恢复。确定吗？`,
    prefsSavedNoEstimate: '设置已保存，但磁盘空间估算暂时不可用',
    prefsSaved: '已保存本地保留与历史补拉设置。',
    prefsSaveFailed: '无法保存设置',
    syncInProgress: '当前有同步进行中，请稍后再补拉',
    backfillYearCap: '\n一年是上限，更早的云端记录不会进入本机。',
    backfillConfirm: (days: number, low: number, high: number, extra: string) =>
      `补拉 ${days} 天大约需要 ${low}–${high} 分钟（估算）。请保持应用打开，可随时取消。${extra}`,
    backfillTightSpace: (message: string, days: number) =>
      `${message}\n仍要按 ${days} 天补拉吗？建议先选 30 天。`,

    // ── 能力清单标签 ──
    stream: {
      heart_rate: '心率',
      sleep: '睡眠',
      workouts: '运动',
      steps: '步数',
      daily_activity: '日常活动',
      stress: '压力',
      spo2: '夜间血氧指标',
      respiratory_rate: '呼吸率',
      hrv: 'HRV (SDNN)',
      hrv_rmssd: 'HRV (RMSSD)',
      recovery: '恢复与能量',
      training_load: '训练负荷',
      vo2max: 'VO₂max',
      lactate_threshold: '乳酸阈值',
      pai: 'PAI 活力指数',
      blood_pressure: '血压',
      weight: '体重',
      emotion: '情绪',
      food: '饮食记录（卡路里与三大营养素）',
      second_heart_rate: '逐秒心率索引',
      spo2_files: '逐条血氧原始文件索引',
    },
    unitDays: '天',
    unitRecords: '条',
    capabilityNoRecords: (days: number) => `最近 ${days} 天没有记录`,
    capabilityNotIngested: '云端有记录，但本机尚无可用数据。请同步或补拉；若仍未收录，可能需要补充报文格式支持。',
    capabilityUnsupported: '你的账号或设备不提供这项数据',
    capabilityNoneProbed: (days: number) => `过去 ${days} 天没有测量记录`,
    capabilityNotProbed: '尚未检测',
    capabilityLocal: (records: number, unit: string, latest: string) =>
      `${records} ${unit}${latest ? ` · 至 ${latest}` : ''}`,
    capabilityCloud: (records: number, unit: string, latest: string) =>
      `云端 ${records} ${unit}${latest ? ` · 至 ${latest}` : ''}`,

    reportCategory: {
      device: { label: '设备没有被识别', hint: '型号显示错误或显示「未识别」' },
      workout: { label: '运动类型没有被识别', hint: '显示成「未知运动」或认错了项目' },
      data: { label: '数据对不上', hint: '某项一直是空，或和 Zepp App 里的数字不一样' },
      other: { label: '其它问题', hint: '在下面写清楚就行' },
    },
  },
  {
    // ── Header ──
    title: 'Settings',
    intro: 'Authentication, sync behavior, privacy, and the export defaults — all in one place.',
    retry: 'Try again',
    distanceUnitLabel: 'Distance unit',
    displayPrefsTitle: 'Language and formats',

    // ── 1. Authentication ──
    authTitle: '1. Authentication',
    authWebTitle: 'Official web sign-in',
    authWebSub: 'Sign in on the official page; the appToken is picked up for you',
    authCancelLogin: 'Cancel sign-in',
    authInUse: 'In use',
    authOpening: 'Opening…',
    authRetry: 'Try connecting again',
    authUse: 'Use',
    authHarTitle: 'HAR import',
    authHarSub: 'For advanced users and debugging: import a HAR file',
    authManualTitle: 'Enter it yourself',
    authManualSub: 'Type in the appToken, user_id and region host',
    authCollapse: 'Collapse',
    manualFormTitle: 'Enter the credentials',
    manualFormHint: 'Take them from a mitmproxy/Charles capture or your browser devtools. Three fields:',
    manualTokenPlaceholder: 'Copy from the apptoken HTTP header',
    manualUserIdPlaceholder: 'Take from the URL path /users/{user_id}/',
    manualSaving: 'Saving…',
    manualSave: 'Save credentials',
    cancel: 'Cancel',

    // ── 2. Account ──
    accountTitle: '2. Account and region',
    accountLine: (region: string, lastSync: string) => `Region ${region} · last sync ${lastSync}`,
    verifyAndSync: 'Verify and sync',
    reauthenticate: 'Sign in again',

    // ── 3. Devices ──
    devicesTitle: '3. Connected devices / data sources',
    identifying: 'Identifying…',
    identifyDevices: 'Identify devices again',
    deviceErrorPrefix: 'Device identification: ',
    noDevices: 'No physical device identified yet; Zepp Cloud still syncs as a cloud source.',
    deviceFirmware: (firmware: string) => `Firmware ${firmware}`,
    deviceLatestData: 'Latest data',
    deviceIdLine: (masked: string) => `Device ID ${masked}`,
    viewOrChange: 'View / change model',
    unknownDeviceTitle: 'An unidentified device',
    unknownDeviceBodyA: 'Some Zepp accounts return device records with ',
    unknownDeviceNoName: 'no product-name field at all',
    unknownDeviceBodyB: ' — only internal numbers, from which no model can be inferred. Pressing "Identify devices again" will never change that. You can point at the model yourself above: it gets labeled "Model you picked" and is never passed off as an automatic match.',
    unknownDeviceReport: 'Submitting an error report helps get this device\'s numbers into the built-in catalog, so nobody has to pick it by hand afterwards. The report carries a fixed whitelist of fields, and needs no GitHub account.',

    // ── Error report form ──
    reportWhat: 'What is wrong',
    reportWhatHint: ' (pick one and you can submit even when nothing was auto-detected)',
    reportCategoryPlaceholder: 'Unspecified (send only what was auto-detected)',
    reportCategoryAria: 'Type of problem to report',
    reportNote: 'Anything to add',
    reportNoteHint: ' (optional, but very useful)',
    reportNotePlaceholder: 'For example: my watch is an Amazfit Balance 2 but it shows as unidentified; or: outdoor cycling was read as an unknown workout.',
    reportNoteCounter: (used: number, max: number) =>
      `${used} / ${max} · local paths, email addresses and long identifiers are stripped before sending`,
    reportSubmitting: 'Submitting…',
    reportSubmit: 'Submit an error report',
    reportDoneTitle: 'Received, thank you',
    reportDoneLine: (id: string, at: string) => `Report ${id}, submitted ${at}.`,
    reportDoneNote: 'What went out is exactly the field types listed above plus the note you wrote. Nothing else.',
    reportConfirm: 'This sends the app version, OS type, parser revision, product-level hints and field shapes for unidentified devices, firmware version, model-level numbers (deviceSource / deviceType — integers describing which model, not which unit), unknown workout codes and their counts, the numeric error code from the most recent request the cloud rejected (just the number, which data stream, and when — never any text the cloud returned), and the note you wrote above (with local paths, email addresses and long identifiers stripped). It never sends your Zepp account, tokens, serial numbers, device IDs, MAC addresses, GPS, health values or raw responses. Submit?',
    reportFailed: 'The error report could not be submitted',

    // ── Capability board ──
    capabilityTitle: 'What your devices can provide',
    capabilityIntro: 'What ZeppBridge can currently read from your account. This list updates itself during a sync; there is nothing to press.',
    lampOn: (count: number) => `Fetched ${count}`,
    lampPending: (count: number) => `In the cloud, not stored locally ${count}`,
    lampOff: (count: number) => `Not fetched ${count}`,
    capabilityEmptyTitle: 'Not synced yet',
    capabilityEmptyBody: 'These light up after one sync.',
    probeSummary: 'Endpoint diagnostics',
    probeNote: '"Not fetched" does not mean the device lacks it: Zepp\'s endpoints return an empty response for streams that do not exist, and only an outright refusal is reported as "your device does not provide this".',
    probing: 'Probing…',
    probeRun: 'Probe again now',
    probedToday: 'probed today',
    probedDaysAgo: (days: number) => `probed ${days} days ago`,
    probeRecords: (records: number, latest: string) => `${records} records${latest ? `, latest ${latest}` : ''}`,
    probeEmpty: 'no data',
    probeRefused: 'endpoint refused',
    probeFailed: 'request failed',

    // ── Unknown workout codes ──
    codesTitle: 'Unidentified workout codes',
    codesUnnamed: (count: number) => `${count} still unnamed`,
    codesIntro: "Zepp's custom training templates give a number and no name, and the built-in catalog has nothing for them either. Rather than guess a sport and hand it to you, name the code once yourself — every record with that code then uses your name, and the workout page says plainly that it is yours.",
    codeNumber: (code: number) => `Zepp code ${code}`,
    codeRecords: (count: number) => `${count} local records get renamed with it`,
    codeShownAs: (label: string) => `Currently shown as "${label}"`,
    codeShownAsUnknown: (code: number) => `Currently shown as "Unrecognized workout (code ${code})"`,
    codeInputAria: (code: number) => `Custom name for code ${code}`,
    codeInputPlaceholder: 'Give it a name, e.g. My core session',
    codeSaving: 'Saving…',
    codeSave: 'Save',
    codeFootnote: 'The name stays on this machine, is never sent back to Zepp, and survives a reparse. Save it empty to clear it.',
    codeSaved: (code: number, label: string) => `Code ${code} now shows as "${label}".`,
    codeCleared: (code: number) => `The custom name for code ${code} was cleared.`,
    codeSaveFailed: 'Could not save the custom workout name',
    codeSuggestions: ['Strength', 'Core', 'HIIT', 'Stretching', 'Rehab', 'Custom session'],

    // ── 4. Privacy ──
    privacyTitle: '4. Privacy and security',
    privacyDbTitle: 'The local database is not encrypted',
    privacyDbBody: 'Health data is stored as plain SQLite in the app\'s data folder, protected by your Windows / macOS account and disk encryption. ZeppBridge does not encrypt the whole database, and does not pretend to.',
    privacyTokenTitle: 'Zepp tokens use the system credential store by default',
    privacyTokenBody: 'The default is Windows Credential Manager / macOS Keychain / Linux keyring. macOS and Linux can explicitly use a plaintext credential file readable and writable only by your user; Linux also supports environment variables. auth.json holds only account and region metadata. Tokens never enter logs, data exports or error reports.',
    privacyTelemetryTitle: 'No telemetry, no usage statistics',
    privacyTelemetryBody: 'The app reports no usage behavior on its own. Only when you press "Submit an error report" yourself does it send the de-identified fields listed below.',
    privacyModalLink: 'Read the local privacy principles',
    privacyReportTitle: 'A device or workout not recognized?',
    privacyReportBody: 'No GitHub account, no copying data. On confirmation it sends only product-level field shapes, firmware version, model-level numbers (integers, saying only which model), and unknown workout codes with their counts, to ZeppBridge\'s private error-report store. It never sends your account, tokens, serial numbers, device IDs, MAC addresses, GPS, health values, raw responses or local paths.',

    // ── 5. MCP ──
    mcpTitle: '5. MCP (let AI tools query your local data)',
    mcpBadge: 'Read-only · listens on no port',
    mcpSkip: 'If MCP means nothing to you, skip this section — it affects none of ZeppBridge\'s features.',
    mcpCompareA: 'In one line: "Hand to AI" is you exporting and pasting; MCP is ',
    mcpCompareStrong: 'the AI asking for itself',
    mcpCompareB: ' — once configured, you say "how did I sleep this month" and it queries your local database. Only useful for AI coding tools installed on your computer (Claude Code, Codex, Grok and the like).',
    mcpAskA: 'The setup differs per tool, so rather than write an essay here, ',
    mcpAskStrong: 'copy the text below to the AI you actually use',
    mcpAskB: ' and let it walk you through it on your own machine.',
    mcpCopyPrompt: 'Copy this and ask your AI',
    mcpCopyConfig: 'Copy just the config snippet',
    mcpToolsLead: 'Once configured, the AI can ask about these five things:',
    mcpFootA: ' ships in the tools archive on each Release, at the same version as the desktop app. It reads the same local database, so what it sees is exactly what you see here.',
    mcpPromptCopied: 'Copied. Paste it to the AI you use and it will give you setup steps for your machine.',
    mcpPromptCopyFailed: 'Copying failed. Select the text above by hand.',
    mcpConfigCopied: 'Config copied. Replace command with the real path to zeppbridge-mcp on your machine.',
    mcpConfigCopyFailed: 'Copying failed. Select the config above by hand.',
    mcpToolListWorkouts: 'Workout list, newest first',
    mcpToolWorkoutInsight: 'One workout against your own baseline',
    mcpToolMetricSeries: 'Day-by-day metric series, each with its unit',
    mcpToolSleepDetail: 'One night of sleep, stage by stage',
    mcpToolDataHealth: 'Fetch/parse/write state for each stream',
    mcpSetupPrompt: `I use a Windows desktop app called ZeppBridge that syncs my Amazfit / Zepp watch data into a local SQLite database.
It ships an MCP program (zeppbridge-mcp) and I want to configure it with you, so you can query my workouts and health data directly instead of me exporting and pasting every time.

What I know about it:
- The MCP program comes from the zeppbridge-tools archive on ZeppBridge's GitHub Releases page; unzip it and zeppbridge-mcp is inside. I may not have downloaded it yet.
- It is a stdio MCP server. It reads the local database, does not use the network, listens on no port, and needs no token or API key.
- The typical config shape is: {"mcpServers": {"zeppbridge": {"command": "<full path to zeppbridge-mcp>", "args": []}}}
- It exposes six read-only tools: list_workouts, get_workout_insight (one workout against my own baseline), get_metric_series (day-by-day metric series), get_sleep_detail (one night, stage by stage), get_sleep_for_date (sleep sessions for a local date and IANA timezone), and get_data_health (fetch/parse/write state per stream).

Please tell me:
1. For you specifically — the tool I am talking to right now — which file the config goes in, or which command adds it;
2. How to write a Windows path (do backslashes need escaping);
3. How to verify it works once configured.

If you need anything from me (which client I use, where the file lives), just ask.`,
    mcpConfigPathPlaceholder: '<path to zeppbridge-mcp>',

    // ── 6. Retention ──
    retentionTitle: '6. Local data retention',
    retentionLabel: 'Keep for',
    retentionAria: 'Local data retention in days',
    retentionNote: (days: number) => `Keeps the last ${days} days locally. Pruning happens `,
    retentionNoteStrong: 'after a successful sync',
    retentionNoteTail: ', never on its own in the background.',
    retentionCutoff: (date: string) => `After the next successful sync, data older than ${date} is pruned`,
    cleaningUp: 'Cleaning up…',
    cleanupNow: 'Clean up now',
    reprocessing: 'Reparsing…',
    reprocessNow: 'Reparse',
    days: (days: number) => `${days} days`,
    lastDays: (days: number) => `Last ${days} days`,

    // ── 7. Export ──
    exportTitle: '7. Export and backfill defaults',
    defaultFormatLabel: 'Default export format',
    defaultFormatAria: 'Default export format',
    historyRangeLabel: 'History backfill range',
    historyRangeAria: 'History backfill days',
    exportNote: 'Sets the default format on the "Hand to AI" page and the cloud backfill window.',
    startBackfill: 'Start a history backfill',
    formatJsonHint: 'Structured data',
    formatCsvHint: 'Tabular data',
    formatGpxHint: 'Workout tracks',

    // ── 8. Updates ──
    updateTitle: '8. Software updates',
    updateSub: 'Checks quietly at most once a day; you can also check by hand.',
    updateChecking: 'Checking…',
    updateCheck: 'Check for updates',
    updateCurrent: (version: string) => `Currently ${version}`,
    updateVersion: (version: string) => `Version ${version}`,
    buildStamp: (stamp: string) => `Build ${stamp}`,
    updateVersionLoading: 'loading',
    updateSeeNotes: 'See what changed',
    updateStatusIdle: 'Not checked yet',
    updateStatusChecking: 'Checking GitHub Releases',
    updateStatusAvailable: (version: string) => `Version ${version} is available`,
    updateStatusDownloading: 'Downloading the update',
    updateStatusDownloadingPercent: (percent: number) => `Downloading ${percent}%`,
    updateStatusInstalling: 'Installing; the app restarts when it is done',
    updateStatusFailed: 'The update failed',
    updateStatusUpToDate: 'You are on the latest version',
    updateStatusUnmanaged: 'Updates come from your package manager',
    updateUnmanagedHint: (version: string) => `On ${version}. This build updates through Flatpak or your `
      + `distribution's package manager: run flatpak update com.zeppbridge.app, or upgrade the package.`,
    releaseNotesEmpty: 'This release carries no notes.',
    updateModalTitle: (version: string) => `What changed in ZeppBridge ${version}`,
    updateModalCurrent: (version: string) => `You are on ${version}`,
    updateModalUnknownVersion: 'an unknown version',
    updateModalReleased: (date: string) => ` · released ${date}`,
    updateInstalling: 'Installing…',
    updateDownloading: 'Downloading the update',
    updateInstallNote: 'The app restarts itself once installed. Local health data is not deleted.',
    updateDownloadNoteTail: ' · it installs automatically when done; you can keep reading the notes above',
    updateDownloadNote: 'It installs automatically when done; you can keep reading the notes above.',
    updateFailedPrefix: (reason: string) => `Update failed: ${reason}`,
    updateRestartNote: 'The app restarts during installation. Local health data is not deleted.',
    updateBackground: 'Continue in the background',
    updateLater: 'Not now',
    updateRetry: 'Try again',
    updateInstall: 'Download and install',

    // ── 9. Auto sync ──
    syncTitle: '9. Automatic sync',
    syncDescA: (minutes: number) => `Syncs cloud records every ${minutes} minutes while the app is open`,
    syncDescB: 'Leaving it on keeps the time series continuous.',
    syncIntervalAria: 'Automatic sync interval',
    minutes: (minutes: number) => `${minutes} min`,
    syncOn: 'Sync is on',
    syncOff: 'Sync is off',
    syncing: 'Syncing…',
    syncNow: 'Sync now',

    // ── Advanced ──
    advancedTitle: 'Advanced and maintenance',
    advancedSub: 'Scaling, the data folder and clearing credentials. Only when you need them.',
    scaleLabel: 'Interface scale',
    scaleNote: '100% is the design baseline. Ctrl + / Ctrl - work too.',
    dataAuthLabel: 'Data and credentials',
    dataAuthNote: (days: number) => `Data lives in the app\'s data folder; currently keeping ${days} days.`,
    openDataFolder: 'Open the data folder',
    clearAuth: 'Clear credentials',
    logout: 'Log out',
    logoutHint: 'Logs out of the account only. Everything already synced to this machine is kept, and syncing resumes after you log back in.',
    logoutNoMultiAccount: 'Switching between accounts is not supported yet: if you log in with a different account afterwards, both accounts write into the same local library.',
    healthCheckLabel: 'Data health check',
    healthCheckNote: 'How far each data stream got through fetching from the cloud, parsing and writing locally; which dates it covers; where it came from. Not something to watch day to day — come here when a sync result does not match what you expected.',
    healthCheckOpen: 'Open the data health check',
    compactLabel: 'Compact stored payloads',
    compactNoteA: 'Raw cloud payloads take up most of this database. They are JSON text, and usually compress to about a fifth.',
    compactNoteStrong: 'This happens automatically',
    compactNoteB: ': the first time a new version starts, the background compacts what is stored, a banner says so, and it disappears when done. This button just runs it again by hand (say, if it was interrupted). Before replacing anything it decompresses and compares byte for byte, skipping any payload that does not match — the raw payload is the only basis for a replay, so it would rather leave it alone. A VACUUM runs afterwards, which is what actually shrinks the file on disk.',
    compacting: 'Compacting… (a few minutes on a large database)',
    compactRun: 'Compact stored payloads',
    backupLabel: 'Database snapshots and restore',
    backupNote: 'A whole-database copy for disaster recovery, readable only by ZeppBridge. One is taken automatically before a database upgrade; you rarely need to do it by hand.',
    localApiLabel: 'Local REST API',
    localApiNote: 'For other programs on this machine — scripts, dashboards, your own tools — to read normalized workout series as JSON. If you have no such need, leave it off.',
    syncDiagnostics: 'Sync diagnostics',
    noSyncDiagnostics: 'No sync diagnostics yet.',

    // ── Local REST API ──
    apiTitle: 'Local REST API',
    apiSub: 'Lets other programs on this machine read normalized workout series as JSON. Off by default; you turn it on explicitly.',
    apiListening: 'Listening',
    apiEnabledNotListening: 'Enabled but not listening',
    apiOff: 'Off',
    apiToggleTitle: 'Enable the local API',
    apiToggleSub: (address: string) => `Takes effect immediately, no restart needed. Turning it off releases ${address} at once.`,
    apiToggleAria: 'Enable the local REST API',
    apiCopyExample: 'Copy an authenticated example',
    apiTokenLabel: 'Access token',
    apiHide: 'Hide',
    apiShow: 'Show',
    apiCopy: 'Copy',
    apiRegenerate: 'Regenerate',
    apiAuthNoteA: 'Every request must carry ',
    apiAuthNoteB: ', or it gets a 401. Regenerating invalidates the old token immediately.',
    apiBindNote: 'Bound to 127.0.0.1 only: read-only, no browser cross-origin access, and it returns no credentials. It stops when you quit ZeppBridge.',
    apiTokenReadFailed: 'Could not read the local API access token',
    apiEnabled: 'The local API is on. No restart needed.',
    apiDisabled: 'The local API is off and the port is released.',
    apiToggleFailed: 'Could not switch the local API',
    apiTokenCopied: 'Access token copied to the clipboard.',
    apiTokenCopyFailed: 'Could not write to the clipboard. Press "Show" and copy it by hand.',
    apiRegenerateConfirm: 'Regenerating invalidates the old token immediately, and every local program configured with it needs updating. Continue?',
    apiTokenRegenerated: 'A new access token was generated. The old one is invalid.',
    apiRegenerateFailed: 'Could not regenerate the access token',
    apiExampleCopied: 'The authenticated example was copied (it contains your access token).',
    apiExampleCopyFailed: 'Could not copy the example. Put the endpoint URL and the Authorization header together by hand.',

    // ── Privacy modal ──
    privacyModalTitle: 'ZeppBridge local privacy principles',
    privacyPoint1Title: '1. Local first: ',
    privacyPoint1: 'all health and workout time series live only in the local SQLite database; parsing and de-identification happen entirely on this machine.',
    privacyPoint2Title: '2. Credentials stay isolated: ',
    privacyPoint2: 'the App Token and User ID are shared with no third party, and an AI export de-identifies them irreversibly.',
    privacyPoint3Title: '3. Location under control: ',
    privacyPoint3: 'GPS coordinates never go to the AI clipboard by default, keeping your home and regular routes private.',
    privacyPoint4Title: '4. Error reports are your call: ',
    privacyPoint4: 'only after you press and confirm "Submit an error report" does it send a fixed whitelist of product-level diagnostics. It never sends your account, device IDs, workout details or health data, and it never opens a GitHub issue for you.',
    privacyPoint5Title: '5. Open source throughout: ',
    privacyPoint5: 'the whole codebase is open, with no hidden phone-home logic.',
    privacyModalOk: 'Got it',
    closeDialog: 'Close dialog',

    // ── Status and notices ──
    connExtracting: 'Extracting the sign-in details',
    connVerifying: 'Verifying',
    connWaiting: 'Waiting for sign-in',
    connFailed: 'Sign-in failed',
    unidentified: 'Not identified',
    unidentifiedInitial: '?',
    notProvided: 'Not provided',
    noRecords: 'No records yet',
    timeUnknown: 'Time unknown',
    cloudService: 'Cloud service',
    refreshFailed: (reason: string) => `Identification failed; fell back to the local cache${reason}`,
    refreshFailedReason: (reason: string) => `: ${reason}`,
    refreshFailedPeriod: '.',
    refreshDone: (count: number) => `Identification finished; ${count} physical devices found.`,
    refreshNoNewList: 'No new device list came back; showing the local cache.',
    loginIncomplete: 'Sign-in did not complete',
    loginWindowFailed: 'Could not open the sign-in window',
    loginCancelFailed: 'Could not cancel the sign-in',
    harFilter: 'HAR file',
    harImported: 'HAR file imported; the credentials are saved.',
    harImportFailed: 'The HAR import failed',
    filePickerFailed: 'Could not open the file picker',
    fillAllFields: 'Fill in every required field',
    manualAuthDone: 'Manual authentication succeeded; the credentials are saved.',
    manualAuthFailed: 'Manual authentication failed',
    verifyFailed: 'Verification did not complete',
    clearAuthConfirm: 'Log out of this account?\n\nOnly the sign-in credentials are cleared; everything already synced to this machine is kept.\n\nNote: switching between accounts is not supported yet — logging in with a different account writes both accounts into the same local library.',
    authCleared: 'Logged out. Everything already synced to this machine is still here.',
    clearAuthFailed: 'Could not clear the credentials',
    reprocessed: (count: number) => `Local data reparsed into ${count} normalized records. The cloud sync time is unchanged.`,
    reprocessFailed: 'Reparsing the local data failed',
    cleanupConfirm: (days: number) => `Clean up local data older than ${days} days? This cannot be undone.`,
    cleanupDone: (days: number) => `Data older than ${days} days was cleaned up.`,
    cleanupFailed: 'Cleaning up old data failed',
    openFolderFailed: 'Could not open the data folder',
    nothingToCompact: 'Nothing to compact — the stored payloads are already compressed.',
    compactSkipped: (count: number) => `, ${count} skipped (no smaller after compression, or the check did not match)`,
    compactDone: (count: number, before: string, after: string, saved: string, skipped: string) =>
      `Compacted ${count} payloads, ${before} → ${after}, saving ${saved}${skipped}.`,
    compactFailed: 'Compacting the stored payloads failed',
    retentionConfirm: (days: number) => `The next successful sync will delete local data older than ${days} days, permanently. Continue?`,
    prefsSavedNoEstimate: 'Settings saved, but the disk space estimate is unavailable right now',
    prefsSaved: 'Retention and backfill settings saved.',
    prefsSaveFailed: 'Could not save the settings',
    syncInProgress: 'A sync is running. Backfill once it finishes',
    backfillYearCap: '\nOne year is the cap; cloud records older than that do not come to this machine.',
    backfillConfirm: (days: number, low: number, high: number, extra: string) =>
      `Backfilling ${days} days takes roughly ${low}–${high} minutes (estimated). Keep the app open; you can cancel at any time.${extra}`,
    backfillTightSpace: (message: string, days: number) =>
      `${message}\nStill backfill ${days} days? Consider 30 days first.`,

    // ── Capability labels ──
    stream: {
      heart_rate: 'Heart rate',
      sleep: 'Sleep',
      workouts: 'Workouts',
      steps: 'Steps',
      daily_activity: 'Daily activity',
      stress: 'Stress',
      spo2: 'Nighttime SpO2 metrics',
      respiratory_rate: 'Respiratory rate',
      hrv: 'HRV (SDNN)',
      hrv_rmssd: 'HRV (RMSSD)',
      recovery: 'Readiness and energy',
      training_load: 'Training load',
      vo2max: 'VO₂max',
      lactate_threshold: 'Lactate threshold',
      pai: 'PAI',
      blood_pressure: 'Blood pressure',
      weight: 'Weight',
      emotion: 'Mood',
      food: 'Food log (calories and macros)',
      second_heart_rate: 'Per-second heart rate index',
      spo2_files: 'Per-reading SpO2 raw file index',
    },
    unitDays: 'days',
    unitRecords: 'records',
    capabilityNoRecords: (days: number) => `Nothing recorded in the last ${days} days`,
    capabilityNotIngested: 'The cloud has records, but no usable data is stored locally yet. Try syncing or backfilling; if records still do not appear, their payload format may need additional support.',
    capabilityUnsupported: 'Your account or device does not provide this',
    capabilityNoneProbed: (days: number) => `No measurement in the last ${days} days`,
    capabilityNotProbed: 'Not probed yet',
    capabilityLocal: (records: number, unit: string, latest: string) =>
      `${records} ${unit}${latest ? ` · through ${latest}` : ''}`,
    capabilityCloud: (records: number, unit: string, latest: string) =>
      `${records} ${unit} in the cloud${latest ? ` · through ${latest}` : ''}`,

    reportCategory: {
      device: { label: 'A device was not recognized', hint: 'the model is wrong, or it shows "Not identified"' },
      workout: { label: 'A workout type was not recognized', hint: 'it shows as an unknown workout, or as the wrong sport' },
      data: { label: 'The numbers do not match', hint: 'something is always empty, or differs from the Zepp app' },
      other: { label: 'Something else', hint: 'describe it below' },
    },
  },
  {
    // ── Header ──
    title: 'Configuración',
    intro: 'Autenticación, sincronización, privacidad y opciones de exportación, todo en un solo lugar.',
    retry: 'Reintentar',
    distanceUnitLabel: 'Unidad de distancia',
    displayPrefsTitle: 'Idioma y formatos',

    // ── 1. Authentication ──
    authTitle: '1. Autenticación',
    authWebTitle: 'Inicio de sesión web oficial',
    authWebSub: 'Inicia sesión en la página oficial; el appToken se obtiene automáticamente',
    authCancelLogin: 'Cancelar inicio de sesión',
    authInUse: 'En uso',
    authOpening: 'Abriendo…',
    authRetry: 'Intentar conectar de nuevo',
    authUse: 'Usar',
    authHarTitle: 'Importar HAR',
    authHarSub: 'Para usuarios avanzados y depuración: importa un archivo HAR',
    authManualTitle: 'Ingresarlo a mano',
    authManualSub: 'Escribe el appToken, el user_id y el host de la región',
    authCollapse: 'Contraer',
    manualFormTitle: 'Ingresa las credenciales',
    manualFormHint: 'Tómalas de una captura de mitmproxy/Charles o de las herramientas de desarrollador del navegador. Son tres campos:',
    manualTokenPlaceholder: 'Cópialo del encabezado HTTP apptoken',
    manualUserIdPlaceholder: 'Tómalo de la ruta de la URL /users/{user_id}/',
    manualSaving: 'Guardando…',
    manualSave: 'Guardar credenciales',
    cancel: 'Cancelar',

    // ── 2. Account ──
    accountTitle: '2. Cuenta y región',
    accountLine: (region: string, lastSync: string) => `Región ${region} · última sincronización ${lastSync}`,
    verifyAndSync: 'Verificar y sincronizar',
    reauthenticate: 'Volver a iniciar sesión',

    // ── 3. Devices ──
    devicesTitle: '3. Dispositivos conectados / fuentes de datos',
    identifying: 'Identificando…',
    identifyDevices: 'Volver a identificar dispositivos',
    deviceErrorPrefix: 'Identificación de dispositivos: ',
    noDevices: 'Aún no se ha identificado ningún dispositivo físico; Zepp Cloud sigue sincronizando como fuente en la nube.',
    deviceFirmware: (firmware: string) => `Firmware ${firmware}`,
    deviceLatestData: 'Últimos datos',
    deviceIdLine: (masked: string) => `ID del dispositivo ${masked}`,
    viewOrChange: 'Ver / cambiar modelo',
    unknownDeviceTitle: 'Un dispositivo sin identificar',
    unknownDeviceBodyA: 'Algunas cuentas de Zepp devuelven registros de dispositivos ',
    unknownDeviceNoName: 'sin ningún campo con el nombre del producto',
    unknownDeviceBodyB: ': solo números internos, de los que no se puede deducir el modelo. Pulsar «Volver a identificar dispositivos» nunca cambiará eso. Puedes indicar el modelo tú mismo arriba: quedará marcado como «Modelo que elegiste» y nunca se presentará como una coincidencia automática.',
    unknownDeviceReport: 'Enviar un reporte de error ayuda a que los números de este dispositivo entren al catálogo integrado, para que nadie tenga que elegirlo a mano después. El reporte lleva una lista fija de campos permitidos y no necesita cuenta de GitHub.',

    // ── Error report form ──
    reportWhat: 'Qué está mal',
    reportWhatHint: ' (elige una opción y podrás enviarlo aunque no se haya detectado nada automáticamente)',
    reportCategoryPlaceholder: 'Sin especificar (enviar solo lo detectado automáticamente)',
    reportCategoryAria: 'Tipo de problema a reportar',
    reportNote: 'Algo que agregar',
    reportNoteHint: ' (opcional, pero muy útil)',
    reportNotePlaceholder: 'Por ejemplo: mi reloj es un Amazfit Balance 2 pero aparece sin identificar; o: el ciclismo al aire libre se leyó como un entrenamiento desconocido.',
    reportNoteCounter: (used: number, max: number) =>
      `${used} / ${max} · las rutas locales, direcciones de correo e identificadores largos se eliminan antes de enviar`,
    reportSubmitting: 'Enviando…',
    reportSubmit: 'Enviar un reporte de error',
    reportDoneTitle: 'Recibido, gracias',
    reportDoneLine: (id: string, at: string) => `Reporte ${id}, enviado el ${at}.`,
    reportDoneNote: 'Lo que se envió son exactamente los tipos de campos listados arriba más la nota que escribiste. Nada más.',
    reportConfirm: 'Esto envía la versión de la app, el tipo de sistema operativo, la revisión del analizador, pistas a nivel de producto y la forma de los campos de dispositivos sin identificar, la versión de firmware, números a nivel de modelo (deviceSource / deviceType: enteros que indican qué modelo, no qué unidad), códigos de entrenamiento desconocidos y cuántas veces aparecen, el código numérico del error de la solicitud más reciente que rechazó la nube (solo el número, qué flujo de datos y cuándo; nunca el texto que devolvió la nube) y la nota que escribiste arriba (sin rutas locales, direcciones de correo ni identificadores largos). Nunca envía tu cuenta de Zepp, tokens, números de serie, IDs de dispositivo, direcciones MAC, GPS, valores de salud ni respuestas originales. ¿Enviar?',
    reportFailed: 'No se pudo enviar el reporte de error',

    // ── Capability board ──
    capabilityTitle: 'Qué pueden aportar tus dispositivos',
    capabilityIntro: 'Lo que ZeppBridge puede leer actualmente de tu cuenta. Esta lista se actualiza sola durante una sincronización; no hay nada que pulsar.',
    lampOn: (count: number) => `Descargados ${count}`,
    lampPending: (count: number) => `En la nube, sin guardar localmente ${count}`,
    lampOff: (count: number) => `Sin descargar ${count}`,
    capabilityEmptyTitle: 'Aún sin sincronizar',
    capabilityEmptyBody: 'Se encienden después de una sincronización.',
    probeSummary: 'Diagnóstico de endpoints',
    probeNote: '«Sin descargar» no significa que el dispositivo no lo tenga: los endpoints de Zepp devuelven una respuesta vacía para los flujos que no existen, y solo un rechazo explícito se reporta como «tu dispositivo no ofrece esto».',
    probing: 'Probando…',
    probeRun: 'Probar de nuevo ahora',
    probedToday: 'probado hoy',
    probedDaysAgo: (days: number) => `probado hace ${days} días`,
    probeRecords: (records: number, latest: string) => `${records} registros${latest ? `, el último ${latest}` : ''}`,
    probeEmpty: 'sin datos',
    probeRefused: 'endpoint rechazado',
    probeFailed: 'la solicitud falló',

    // ── Unknown workout codes ──
    codesTitle: 'Códigos de entrenamiento sin identificar',
    codesUnnamed: (count: number) => `${count} aún sin nombre`,
    codesIntro: 'Las plantillas de entrenamiento personalizadas de Zepp dan un número sin nombre, y el catálogo integrado tampoco tiene nada para ellas. En lugar de adivinar un deporte y dártelo, ponle nombre al código una sola vez: todos los registros con ese código usarán tu nombre, y la página del entrenamiento dirá claramente que es tuyo.',
    codeNumber: (code: number) => `Código de Zepp ${code}`,
    codeRecords: (count: number) => `${count} registros locales se renombran con él`,
    codeShownAs: (label: string) => `Se muestra como «${label}»`,
    codeShownAsUnknown: (code: number) => `Se muestra como «Entrenamiento no reconocido (código ${code})»`,
    codeInputAria: (code: number) => `Nombre personalizado para el código ${code}`,
    codeInputPlaceholder: 'Ponle un nombre, p. ej. Mi sesión de core',
    codeSaving: 'Guardando…',
    codeSave: 'Guardar',
    codeFootnote: 'El nombre se queda en este equipo, nunca se envía a Zepp y se conserva al volver a analizar los datos. Guárdalo vacío para borrarlo.',
    codeSaved: (code: number, label: string) => `El código ${code} ahora se muestra como «${label}».`,
    codeCleared: (code: number) => `Se borró el nombre personalizado del código ${code}.`,
    codeSaveFailed: 'No se pudo guardar el nombre personalizado del entrenamiento',
    codeSuggestions: ['Fuerza', 'Core', 'HIIT', 'Estiramiento', 'Rehabilitación', 'Sesión personalizada'],

    // ── 4. Privacy ──
    privacyTitle: '4. Privacidad y seguridad',
    privacyDbTitle: 'La base de datos local no está cifrada',
    privacyDbBody: 'Los datos de salud se guardan como SQLite sin cifrar en la carpeta de datos de la app, protegidos por tu cuenta de Windows / macOS y el cifrado del disco. ZeppBridge no cifra la base de datos completa, y no pretende hacerlo.',
    privacyTokenTitle: 'Los tokens de Zepp usan el almacén de credenciales del sistema por defecto',
    privacyTokenBody: 'Por defecto se usa el Administrador de credenciales de Windows / el Llavero de macOS / el llavero de Linux. En macOS y Linux se puede usar explícitamente un archivo de credenciales en texto plano que solo tu usuario puede leer y escribir; Linux también admite variables de entorno. auth.json solo guarda metadatos de la cuenta y la región. Los tokens nunca entran en los registros, las exportaciones de datos ni los reportes de error.',
    privacyTelemetryTitle: 'Sin telemetría ni estadísticas de uso',
    privacyTelemetryBody: 'La app no reporta nada sobre tu uso por su cuenta. Solo cuando tú mismo pulsas «Enviar un reporte de error» envía los campos anonimizados que se listan abajo.',
    privacyModalLink: 'Leer los principios de privacidad local',
    privacyReportTitle: '¿Un dispositivo o entrenamiento no reconocido?',
    privacyReportBody: 'Sin cuenta de GitHub y sin copiar datos. Al confirmar, envía solo la forma de los campos a nivel de producto, la versión de firmware, números a nivel de modelo (enteros que solo indican qué modelo) y los códigos de entrenamiento desconocidos con cuántas veces aparecen, al almacén privado de reportes de error de ZeppBridge. Nunca envía tu cuenta, tokens, números de serie, IDs de dispositivo, direcciones MAC, GPS, valores de salud, respuestas originales ni rutas locales.',

    // ── 5. MCP ──
    mcpTitle: '5. MCP (deja que herramientas de IA consulten tus datos locales)',
    mcpBadge: 'Solo lectura · no abre ningún puerto',
    mcpSkip: 'Si MCP no te dice nada, sáltate esta sección: no afecta ninguna función de ZeppBridge.',
    mcpCompareA: 'En una línea: «Pasar a la IA» es exportar y pegar tú mismo; MCP es ',
    mcpCompareStrong: 'la IA pidiendo los datos por su cuenta',
    mcpCompareB: ': una vez configurado, dices «¿cómo dormí este mes?» y consulta tu base de datos local. Solo sirve con herramientas de IA para programar instaladas en tu computador (Claude Code, Codex, Grok y similares).',
    mcpAskA: 'La configuración cambia según la herramienta, así que en vez de escribir un ensayo aquí, ',
    mcpAskStrong: 'copia el texto de abajo en la IA que realmente usas',
    mcpAskB: ' y deja que te guíe en tu propio equipo.',
    mcpCopyPrompt: 'Copiar esto y preguntarle a tu IA',
    mcpCopyConfig: 'Copiar solo el fragmento de configuración',
    mcpToolsLead: 'Una vez configurado, la IA puede preguntar por estas cinco cosas:',
    mcpFootA: ' viene en el archivo de herramientas de cada versión publicada, con la misma versión que la app de escritorio. Lee la misma base de datos local, así que ve exactamente lo mismo que ves aquí.',
    mcpPromptCopied: 'Copiado. Pégalo en la IA que usas y te dará los pasos de configuración para tu equipo.',
    mcpPromptCopyFailed: 'No se pudo copiar. Selecciona el texto de arriba a mano.',
    mcpConfigCopied: 'Configuración copiada. Reemplaza command por la ruta real de zeppbridge-mcp en tu equipo.',
    mcpConfigCopyFailed: 'No se pudo copiar. Selecciona la configuración de arriba a mano.',
    mcpToolListWorkouts: 'Lista de entrenamientos, del más reciente al más antiguo',
    mcpToolWorkoutInsight: 'Un entrenamiento frente a tu propia referencia',
    mcpToolMetricSeries: 'Series de métricas día a día, cada una con su unidad',
    mcpToolSleepDetail: 'Una noche de sueño, fase por fase',
    mcpToolDataHealth: 'Estado de descarga/análisis/escritura de cada flujo',
    mcpSetupPrompt: `Uso una app de escritorio llamada ZeppBridge que sincroniza los datos de mi reloj Amazfit / Zepp en una base de datos SQLite local.
Incluye un programa MCP (zeppbridge-mcp) y quiero configurarlo contigo, para que puedas consultar mis entrenamientos y datos de salud directamente en vez de que yo exporte y pegue cada vez.

Lo que sé:
- El programa MCP viene en el archivo zeppbridge-tools de la página de Releases de ZeppBridge en GitHub; al descomprimirlo, zeppbridge-mcp está adentro. Puede que todavía no lo haya descargado.
- Es un servidor MCP por stdio. Lee la base de datos local, no usa la red, no abre ningún puerto y no necesita token ni clave de API.
- La forma típica de la configuración es: {"mcpServers": {"zeppbridge": {"command": "<ruta completa a zeppbridge-mcp>", "args": []}}}
- Ofrece cinco herramientas de solo lectura: list_workouts, get_workout_insight (un entrenamiento frente a mi propia referencia), get_metric_series (series de métricas día a día), get_sleep_detail (una noche, fase por fase) y get_data_health (estado de descarga/análisis/escritura por flujo).

Por favor dime:
1. Para ti específicamente (la herramienta con la que estoy hablando ahora), en qué archivo va la configuración o qué comando la agrega;
2. Cómo escribir la ruta en mi sistema operativo (por ejemplo, si en Windows hay que escapar las barras invertidas);
3. Cómo verificar que funciona una vez configurado.

Si necesitas algo de mí (qué cliente uso, dónde está el archivo), pregúntame.`,
    mcpConfigPathPlaceholder: '<ruta a zeppbridge-mcp>',

    // ── 6. Retention ──
    retentionTitle: '6. Conservación de datos locales',
    retentionLabel: 'Conservar durante',
    retentionAria: 'Días de conservación de datos locales',
    retentionNote: (days: number) => `Conserva localmente los últimos ${days} días. La limpieza ocurre `,
    retentionNoteStrong: 'después de una sincronización exitosa',
    retentionNoteTail: ', nunca por su cuenta en segundo plano.',
    retentionCutoff: (date: string) => `Después de la próxima sincronización exitosa se borrarán los datos anteriores al ${date}`,
    cleaningUp: 'Limpiando…',
    cleanupNow: 'Limpiar ahora',
    reprocessing: 'Volviendo a analizar…',
    reprocessNow: 'Volver a analizar',
    days: (days: number) => `${days} días`,
    lastDays: (days: number) => `Últimos ${days} días`,

    // ── 7. Export ──
    exportTitle: '7. Opciones de exportación y recuperación de historial',
    defaultFormatLabel: 'Formato de exportación predeterminado',
    defaultFormatAria: 'Formato de exportación predeterminado',
    historyRangeLabel: 'Rango de recuperación de historial',
    historyRangeAria: 'Días de recuperación de historial',
    exportNote: 'Define el formato predeterminado de la página «Pasar a la IA» y el periodo de recuperación desde la nube.',
    startBackfill: 'Recuperar historial',
    formatJsonHint: 'Datos estructurados',
    formatCsvHint: 'Datos en tabla',
    formatGpxHint: 'Recorridos de entrenamientos',

    // ── 8. Updates ──
    updateTitle: '8. Actualizaciones',
    updateSub: 'Revisa en silencio como máximo una vez al día; también puedes revisar a mano.',
    updateChecking: 'Revisando…',
    updateCheck: 'Buscar actualizaciones',
    updateCurrent: (version: string) => `Versión actual ${version}`,
    updateVersion: (version: string) => `Versión ${version}`,
    buildStamp: (stamp: string) => `Compilación ${stamp}`,
    updateVersionLoading: 'cargando',
    updateSeeNotes: 'Ver qué cambió',
    updateStatusIdle: 'Aún sin revisar',
    updateStatusChecking: 'Revisando las versiones publicadas en GitHub',
    updateStatusAvailable: (version: string) => `La versión ${version} está disponible`,
    updateStatusDownloading: 'Descargando la actualización',
    updateStatusDownloadingPercent: (percent: number) => `Descargando ${percent}%`,
    updateStatusInstalling: 'Instalando; la app se reinicia al terminar',
    updateStatusFailed: 'La actualización falló',
    updateStatusUpToDate: 'Tienes la versión más reciente',
    updateStatusUnmanaged: 'Las actualizaciones las gestiona tu gestor de paquetes',
    updateUnmanagedHint: (version: string) => `Versión ${version}. Esta compilación se actualiza con Flatpak o con el gestor `
      + `de paquetes de tu distribución: ejecuta flatpak update com.zeppbridge.app o actualiza el paquete.`,
    releaseNotesEmpty: 'Esta versión no trae notas.',
    updateModalTitle: (version: string) => `Qué cambió en ZeppBridge ${version}`,
    updateModalCurrent: (version: string) => `Tienes la versión ${version}`,
    updateModalUnknownVersion: 'una versión desconocida',
    updateModalReleased: (date: string) => ` · publicada el ${date}`,
    updateInstalling: 'Instalando…',
    updateDownloading: 'Descargando la actualización',
    updateInstallNote: 'La app se reinicia sola una vez instalada. Los datos de salud locales no se borran.',
    updateDownloadNoteTail: ' · se instala automáticamente al terminar; puedes seguir leyendo las notas de arriba',
    updateDownloadNote: 'Se instala automáticamente al terminar; puedes seguir leyendo las notas de arriba.',
    updateFailedPrefix: (reason: string) => `La actualización falló: ${reason}`,
    updateRestartNote: 'La app se reinicia durante la instalación. Los datos de salud locales no se borran.',
    updateBackground: 'Continuar en segundo plano',
    updateLater: 'Ahora no',
    updateRetry: 'Reintentar',
    updateInstall: 'Descargar e instalar',

    // ── 9. Auto sync ──
    syncTitle: '9. Sincronización automática',
    syncDescA: (minutes: number) => `Sincroniza los registros de la nube cada ${minutes} minutos mientras la app está abierta`,
    syncDescB: 'Dejarla activada mantiene continuas las series de tiempo.',
    syncIntervalAria: 'Intervalo de sincronización automática',
    minutes: (minutes: number) => `${minutes} min`,
    syncOn: 'Sincronización activada',
    syncOff: 'Sincronización desactivada',
    syncing: 'Sincronizando…',
    syncNow: 'Sincronizar ahora',

    // ── Advanced ──
    advancedTitle: 'Avanzado y mantenimiento',
    advancedSub: 'Escala, carpeta de datos y borrado de credenciales. Solo cuando lo necesites.',
    scaleLabel: 'Escala de la interfaz',
    scaleNote: '100% es la base del diseño. Ctrl + / Ctrl - también funcionan.',
    dataAuthLabel: 'Datos y credenciales',
    dataAuthNote: (days: number) => `Los datos están en la carpeta de datos de la app; actualmente se conservan ${days} días.`,
    openDataFolder: 'Abrir la carpeta de datos',
    clearAuth: 'Borrar credenciales',
    logout: 'Cerrar sesión',
    logoutHint: 'Cierra solo la sesión de la cuenta. Todo lo ya sincronizado en este equipo se conserva, y la sincronización se reanuda cuando vuelvas a iniciar sesión.',
    logoutNoMultiAccount: 'Todavía no se puede cambiar de cuenta: si después inicias sesión con otra cuenta, ambas escriben en la misma biblioteca local.',
    healthCheckLabel: 'Revisión del estado de los datos',
    healthCheckNote: 'Hasta dónde llegó cada flujo de datos en la descarga desde la nube, el análisis y la escritura local; qué fechas cubre; de dónde vino. No es algo para mirar a diario: ven aquí cuando el resultado de una sincronización no sea el que esperabas.',
    healthCheckOpen: 'Abrir la revisión del estado de los datos',
    compactLabel: 'Compactar registros guardados',
    compactNoteA: 'Los registros originales de la nube ocupan la mayor parte de esta base de datos. Son texto JSON y normalmente se comprimen a una quinta parte.',
    compactNoteStrong: 'Esto ocurre automáticamente',
    compactNoteB: ': la primera vez que arranca una versión nueva, se compacta en segundo plano lo guardado, un aviso lo indica y desaparece al terminar. Este botón solo lo vuelve a ejecutar a mano (por ejemplo, si se interrumpió). Antes de reemplazar algo, descomprime y compara byte por byte, y omite cualquier registro que no coincida: el registro original es la única base para reprocesar, así que prefiere no tocarlo. Después se ejecuta un VACUUM, que es lo que realmente reduce el archivo en disco.',
    compacting: 'Compactando… (unos minutos en una base de datos grande)',
    compactRun: 'Compactar registros guardados',
    backupLabel: 'Copias de la base de datos y restauración',
    backupNote: 'Una copia completa de la base de datos para recuperarse de desastres, legible solo por ZeppBridge. Se hace una automáticamente antes de actualizar la base de datos; rara vez necesitas hacerla a mano.',
    localApiLabel: 'API REST local',
    localApiNote: 'Para que otros programas de este equipo (scripts, tableros, tus propias herramientas) lean series de entrenamientos normalizadas en JSON. Si no lo necesitas, déjala desactivada.',
    syncDiagnostics: 'Diagnóstico de sincronización',
    noSyncDiagnostics: 'Aún no hay diagnóstico de sincronización.',

    // ── Local REST API ──
    apiTitle: 'API REST local',
    apiSub: 'Permite que otros programas de este equipo lean series de entrenamientos normalizadas en JSON. Desactivada por defecto; la activas tú de forma explícita.',
    apiListening: 'Escuchando',
    apiEnabledNotListening: 'Activada pero sin escuchar',
    apiOff: 'Desactivada',
    apiToggleTitle: 'Activar la API local',
    apiToggleSub: (address: string) => `Se aplica de inmediato, sin reiniciar. Al desactivarla se libera ${address} al instante.`,
    apiToggleAria: 'Activar la API REST local',
    apiCopyExample: 'Copiar un ejemplo autenticado',
    apiTokenLabel: 'Token de acceso',
    apiHide: 'Ocultar',
    apiShow: 'Mostrar',
    apiCopy: 'Copiar',
    apiRegenerate: 'Regenerar',
    apiAuthNoteA: 'Cada solicitud debe llevar ',
    apiAuthNoteB: '; si no, recibe un 401. Regenerarlo invalida el token anterior de inmediato.',
    apiBindNote: 'Solo escucha en 127.0.0.1: solo lectura, sin acceso desde otros orígenes del navegador y sin devolver credenciales. Se detiene cuando cierras ZeppBridge.',
    apiTokenReadFailed: 'No se pudo leer el token de acceso de la API local',
    apiEnabled: 'La API local está activada. No hace falta reiniciar.',
    apiDisabled: 'La API local está desactivada y el puerto quedó libre.',
    apiToggleFailed: 'No se pudo cambiar el estado de la API local',
    apiTokenCopied: 'Token de acceso copiado al portapapeles.',
    apiTokenCopyFailed: 'No se pudo escribir en el portapapeles. Pulsa «Mostrar» y cópialo a mano.',
    apiRegenerateConfirm: 'Regenerarlo invalida el token anterior de inmediato, y habrá que actualizar cada programa local que lo use. ¿Continuar?',
    apiTokenRegenerated: 'Se generó un nuevo token de acceso. El anterior ya no es válido.',
    apiRegenerateFailed: 'No se pudo regenerar el token de acceso',
    apiExampleCopied: 'Se copió el ejemplo autenticado (contiene tu token de acceso).',
    apiExampleCopyFailed: 'No se pudo copiar el ejemplo. Junta a mano la URL del endpoint y el encabezado Authorization.',

    // ── Privacy modal ──
    privacyModalTitle: 'Principios de privacidad local de ZeppBridge',
    privacyPoint1Title: '1. Primero lo local: ',
    privacyPoint1: 'todas las series de salud y entrenamiento viven solo en la base de datos SQLite local; el análisis y la anonimización ocurren por completo en este equipo.',
    privacyPoint2Title: '2. Credenciales aisladas: ',
    privacyPoint2: 'el App Token y el User ID no se comparten con terceros, y una exportación para IA los anonimiza de forma irreversible.',
    privacyPoint3Title: '3. Ubicación bajo control: ',
    privacyPoint3: 'las coordenadas GPS nunca van al portapapeles para la IA por defecto, así tu casa y tus rutas habituales se mantienen privadas.',
    privacyPoint4Title: '4. Los reportes de error los decides tú: ',
    privacyPoint4: 'solo después de que pulses y confirmes «Enviar un reporte de error» se envía una lista fija de diagnósticos a nivel de producto. Nunca envía tu cuenta, IDs de dispositivo, detalles de entrenamientos ni datos de salud, y nunca abre un issue de GitHub por ti.',
    privacyPoint5Title: '5. Código abierto de principio a fin: ',
    privacyPoint5: 'todo el código es abierto, sin lógica oculta que envíe datos a casa.',
    privacyModalOk: 'Entendido',
    closeDialog: 'Cerrar diálogo',

    // ── Status and notices ──
    connExtracting: 'Obteniendo los datos de inicio de sesión',
    connVerifying: 'Verificando',
    connWaiting: 'Esperando el inicio de sesión',
    connFailed: 'El inicio de sesión falló',
    unidentified: 'Sin identificar',
    unidentifiedInitial: '?',
    notProvided: 'Sin datos',
    noRecords: 'Aún no hay registros',
    timeUnknown: 'Hora desconocida',
    cloudService: 'Servicio en la nube',
    refreshFailed: (reason: string) => `La identificación falló; se usó la caché local${reason}`,
    refreshFailedReason: (reason: string) => `: ${reason}`,
    refreshFailedPeriod: '.',
    refreshDone: (count: number) => `Identificación terminada; se encontraron ${count} dispositivos físicos.`,
    refreshNoNewList: 'No llegó una lista de dispositivos nueva; se muestra la caché local.',
    loginIncomplete: 'El inicio de sesión no se completó',
    loginWindowFailed: 'No se pudo abrir la ventana de inicio de sesión',
    loginCancelFailed: 'No se pudo cancelar el inicio de sesión',
    harFilter: 'Archivo HAR',
    harImported: 'Archivo HAR importado; las credenciales quedaron guardadas.',
    harImportFailed: 'La importación del HAR falló',
    filePickerFailed: 'No se pudo abrir el selector de archivos',
    fillAllFields: 'Completa todos los campos obligatorios',
    manualAuthDone: 'La autenticación manual funcionó; las credenciales quedaron guardadas.',
    manualAuthFailed: 'La autenticación manual falló',
    verifyFailed: 'La verificación no se completó',
    clearAuthConfirm: '¿Cerrar la sesión de esta cuenta?\n\nSolo se borran las credenciales de inicio de sesión; todo lo ya sincronizado en este equipo se conserva.\n\nNota: todavía no se puede cambiar de cuenta. Si inicias sesión con otra cuenta, ambas escriben en la misma biblioteca local.',
    authCleared: 'Sesión cerrada. Todo lo ya sincronizado en este equipo sigue aquí.',
    clearAuthFailed: 'No se pudieron borrar las credenciales',
    reprocessed: (count: number) => `Los datos locales se volvieron a analizar en ${count} registros normalizados. La hora de sincronización con la nube no cambia.`,
    reprocessFailed: 'Falló el nuevo análisis de los datos locales',
    cleanupConfirm: (days: number) => `¿Borrar los datos locales de hace más de ${days} días? No se puede deshacer.`,
    cleanupDone: (days: number) => `Se borraron los datos de hace más de ${days} días.`,
    cleanupFailed: 'Falló la limpieza de datos antiguos',
    openFolderFailed: 'No se pudo abrir la carpeta de datos',
    nothingToCompact: 'No hay nada que compactar: los registros guardados ya están comprimidos.',
    compactSkipped: (count: number) => `, ${count} omitidos (no quedaban más pequeños al comprimir o la comprobación no coincidió)`,
    compactDone: (count: number, before: string, after: string, saved: string, skipped: string) =>
      `Se compactaron ${count} registros, ${before} → ${after}, ahorrando ${saved}${skipped}.`,
    compactFailed: 'Falló la compactación de los registros guardados',
    retentionConfirm: (days: number) => `La próxima sincronización exitosa borrará de forma permanente los datos locales de hace más de ${days} días. ¿Continuar?`,
    prefsSavedNoEstimate: 'Configuración guardada, pero la estimación de espacio en disco no está disponible en este momento',
    prefsSaved: 'Configuración de conservación y recuperación guardada.',
    prefsSaveFailed: 'No se pudo guardar la configuración',
    syncInProgress: 'Hay una sincronización en curso. Recupera el historial cuando termine',
    backfillYearCap: '\nEl límite es un año; los registros de la nube más antiguos no llegan a este equipo.',
    backfillConfirm: (days: number, low: number, high: number, extra: string) =>
      `Recuperar ${days} días toma aproximadamente ${low}–${high} minutos (estimado). Mantén la app abierta; puedes cancelar en cualquier momento.${extra}`,
    backfillTightSpace: (message: string, days: number) =>
      `${message}\n¿Recuperar igual ${days} días? Considera empezar con 30 días.`,

    // ── Capability labels ──
    stream: {
      heart_rate: 'Frecuencia cardíaca',
      sleep: 'Sueño',
      workouts: 'Entrenamientos',
      steps: 'Pasos',
      daily_activity: 'Actividad diaria',
      stress: 'Estrés',
      spo2: 'Métricas de SpO2 nocturno',
      respiratory_rate: 'Frecuencia respiratoria',
      hrv: 'VFC (SDNN)',
      hrv_rmssd: 'VFC (RMSSD)',
      recovery: 'Recuperación y energía',
      training_load: 'Carga de entrenamiento',
      vo2max: 'VO₂máx',
      lactate_threshold: 'Umbral de lactato',
      pai: 'PAI',
      blood_pressure: 'Presión arterial',
      weight: 'Peso',
      emotion: 'Estado de ánimo',
      food: 'Registro de comidas (calorías y macronutrientes)',
      second_heart_rate: 'Índice de frecuencia cardíaca por segundo',
      spo2_files: 'Índice de archivos originales de SpO2 por lectura',
    },
    unitDays: 'días',
    unitRecords: 'registros',
    capabilityNoRecords: (days: number) => `Nada registrado en los últimos ${days} días`,
    capabilityNotIngested: 'La nube tiene registros, pero todavía no hay datos utilizables guardados localmente. Prueba a sincronizar o recuperar el historial; si los registros siguen sin aparecer, puede que su formato necesite soporte adicional.',
    capabilityUnsupported: 'Tu cuenta o dispositivo no ofrece esto',
    capabilityNoneProbed: (days: number) => `Sin mediciones en los últimos ${days} días`,
    capabilityNotProbed: 'Aún sin probar',
    capabilityLocal: (records: number, unit: string, latest: string) =>
      `${records} ${unit}${latest ? ` · hasta ${latest}` : ''}`,
    capabilityCloud: (records: number, unit: string, latest: string) =>
      `${records} ${unit} en la nube${latest ? ` · hasta ${latest}` : ''}`,

    reportCategory: {
      device: { label: 'No se reconoció un dispositivo', hint: 'el modelo es incorrecto, o aparece «Sin identificar»' },
      workout: { label: 'No se reconoció un tipo de entrenamiento', hint: 'aparece como entrenamiento desconocido, o como otro deporte' },
      data: { label: 'Los números no coinciden', hint: 'algo siempre está vacío, o no coincide con la app Zepp' },
      other: { label: 'Otra cosa', hint: 'descríbelo abajo' },
    },
  },
  'views/Settings',
);
